...
> 基准路径: `.claude/skills/scraping-specialist/references/domains/ida-pro-skills/ida-domain-scripting/`
...
2. **Check API_REFERENCE.md exists** - Always check that $SKILL_DIR/references/api-reference.md exists. Inform the user to run
...
For comprehensive IDA Domain API documentation, see [api-reference.md](references/api-reference.md):
...
Codex: I'll count the functions. Let me analyze the binary...
...
Codex: I'll find all callers of malloc...
[Writes: <work_dir>/script.py]
[Runs: cd $SKILL_DIR && uv run python run.py <work_dir>/script.py -f binary]
...
Codex: I'll decompile main and save the output...
[Writes: <work_dir>/script.py]
[Runs: cd $SKILL_DIR && uv run python run.py <work_dir>/script.py -f binary]
